...
public class EditableCCombo extends Composite {
     ...
     * @see Widget#getStyle()
     */
    public  EditableCCombo( Composite parent, int style ) {
        ...
        listener = new Listener() {
            public   void handleEvent( Event event ) {
                ...
                if( EditableCCombo.this == event.widget ) {
                    comboEvent( event );
                    return;
                }
        ...
        filter = new Listener() {
            public void handleEvent( Event event ) {
                ...
                if( shell == EditableCCombo.this.getShell() ) {
                    handleFocus( SWT.FocusOut );
                }
            }
        };
    ...
    public  void copy() {
     ...
     void createPopup( String[] items, int[] selectionIndices ) {
        ...
        if( selectionIndices.length != 0 ) list.setSelection( selectionIndices );//
    }
    ...
    public  void deselectAll() {
     ...
     void dropDown( boolean drop ) {
        ...
        /*
         * Add a filter to listen to scrolling of the parent composite, when the drop-down is visible. Remove the filter
         * when drop-down is not visible.
         */
        display.removeFilter( SWT.Selection, filter );
    ...
    public  boolean isFocusControl() {
     ...
     boolean isParentScrolling( Control scrollableParent ) {
        ...
        while (parent != null) {
            if( parent.equals( scrollableParent ) ) return true;
     ...
     void listEvent( Event event ) {
        ...
        case SWT.Selection: {
            ...
            text.setText( this.getSelectionText( indices ) );
            text.selectAll();
    ...
    private  String getSelectionText( int[] indices ) {
        String text = "";
        for( int i = 0; i < indices.length; i++ ) {
        	if(indices[i]==-1)
        		continue;
            text += list.getItem( indices[i] );
            if( i != (indices.length - 1) ) text += ",";
        }
        return text;
    }
    ...
    public  boolean traverse( int event ) {
        /*
         * When the traverse event is sent to the CCombo, it will create a list of controls to tab to next. Since the
         * CCombo is a composite, the next control is the Text field which is a child of the CCombo. It will set focus
         * to the text field which really is itself. So, call the traverse next events directly on the text.
         */
