package com.jiuqi.etl.rpa.ui.mouse.click;
...
import com.jiuqi.etl.rpa.runtime.MouseClickTypePackage;
import com.jiuqi.etl.rpa.runtime.MouseKeyPackage;
import com.jiuqi.etl.rpa.runtime.PositionPackage;
import com.jiuqi.etl.rpa.runtime.mouse.click.MouseClickTaskModel;
...
public class MouseClickProperty implements IPropertySource {

	private MouseClickTaskModel model;
	private IPropertyDescriptor[] pds;
	private String[] integerParamStrs;
	private ParameterModel[] integerParams;
	private static final String KEY_CLICKTYPE = "点击类型";
	private static final String KEY_MOUSEKEY = "鼠标按键"; 
	...
	public MouseClickProperty(TaskContext context) {
		this.model = (MouseClickTaskModel) context.getTaskModel();
		...
		ComboBoxPropertyDescriptor clickType = new ComboBoxPropertyDescriptor(KEY_CLICKTYPE, KEY_CLICKTYPE, MouseClickTypePackage.getTitleArray());
		ComboBoxPropertyDescriptor mouseKey = new ComboBoxPropertyDescriptor(KEY_MOUSEKEY, KEY_MOUSEKEY, MouseKeyPackage.getTitleArray());
		...
		clickType.setCategory("输入");
		mouseKey.setCategory("输入");
		...
		TextPropertyDescriptor targetDelay = new TextPropertyDescriptor(KEY_TARGETDELAY, KEY_TARGETDELAY);
		...
		pds = new IPropertyDescriptor[]{/*输入*/clickType,mouseKey,clickMode,
	...
	public Object getPropertyValue(Object id) {
		if(id.equals(KEY_CLICKTYPE)) {
			int index = MouseClickTypePackage.getIndex(this.model.getClickType());
			return index;
		} else if(id.equals(KEY_MOUSEKEY)) {
			int index = MouseKeyPackage.getIndex(this.model.getMouseKey());
			return index;
		} else if(id.equals(KEY_CLICKMODEL)){
			...
			return this.model.getCursorPosition().getOffsetX();
		} else if(id.equals(KEY_OFFSETY)) {
			return this.model.getCursorPosition().getOffsetY();
	...
	public void setPropertyValue(Object id, Object value) {
		if(id.equals(KEY_CLICKTYPE)) {
			this.model.setClickType(MouseClickTypePackage.getMouseClickType((Integer) value));
		} else if(id.equals(KEY_MOUSEKEY)) {
			this.model.setMouseKey(MouseKeyPackage.getValue((Integer) value));
		} else if(id.equals(KEY_CLICKMODEL)){
